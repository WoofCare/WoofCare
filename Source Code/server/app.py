    ...
    try:
        if not firebase_admin._apps:
            if os.path.exists(CREDENTIALS_PATH):
                ...
                print(
                    f"Warning: {CREDENTIALS_PATH} not found. Firebase features will be disabled."
                )
        else:
            db = firestore.client()
...
def extract_content(soup):
    ...
    content_candidates = [
        soup.find("article"),
        soup.find(
            "div", class_=re.compile(r"content|article|post-body|entry-content", re.I)
        ),
        soup.find("main"),
...
def scrape_ngos_endpoint():
    ...
    try:
        ...
        return jsonify({"success": True, "count": len(data), "ngos": data})

    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500


@app.route("/api/import-locations", methods=["GET"])
def import_locations_endpoint():
    if not db:
        return jsonify({"error": "Firebase not initialized"}), 503

    try:
        # Path to the JSON file relative to this script
        json_path = os.path.join(os.path.dirname(__file__), "org_data.json")
        ...
        for org in orgs:
            ...
            # Transform data to match Dart model
            location_data = {
                "name": org.get("Organization", "").lower(),
                "type": org.get("Type of Organization", "").lower(),
                "website": org.get("Website", ""),
                "phone": str(org.get("Phone Number", "")),  # Ensure string
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Successfully imported {total_imported} locations",
